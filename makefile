...
restore-database: ## restore database from backup (usage: make restore-database BACKUP_PREFIX=backup-2024-01-15-120000 [REWRITE_URLS=1] [REWRITE_FROM=http://127.0.0.1:54321] [REWRITE_TO=http://192.168.1.97:54321])
	@if [ -z "$$BACKUP_PREFIX" ]; then \
		...
		echo "Optional URL rewrite: REWRITE_URLS=1 REWRITE_FROM=http://127.0.0.1:54321 REWRITE_TO=http://192.168.1.97:54321"; \
	...
	@if [ -n "$$REWRITE_URLS" ]; then \
		if [ -z "$$REWRITE_FROM" ] || [ -z "$$REWRITE_TO" ]; then \
			...
			echo "Example: REWRITE_URLS=1 REWRITE_FROM=http://127.0.0.1:54321 REWRITE_TO=http://192.168.1.97:54321"; \
			exit 1; \
		fi; \
